...
function App() {
  ...
  const normalizeEventForUndo = (event) => ({
    id: event.originalId || event.id,
    title: event.title || '',
    date: event.date || '',
    backgroundColor: event.backgroundColor || '#a855f7',
    borderColor: event.borderColor || '#a855f7',
    allDay: true,
  ...
  const pushUndoState = () => {
    const snapshot = events
      .filter((event) => event && (event.originalId || event.id))
      .reduce((acc, event) => {
        const key = event.originalId || event.id;
        acc[key] = normalizeEventForUndo(event);
        return acc;
      }, {});

    setUndoStack((prev) => [...prev, Object.values(snapshot)].slice(-15));
  };

  const restoreEventSnapshot = async (snapshot) => {
    const snapshotMap = new Map(snapshot.map((event) => [String(event.id), event]));
    const querySnapshot = await getDocs(collection(db, "events"));
    const existingMap = new Map(querySnapshot.docs.map((docSnap) => [docSnap.id, docSnap.ref]));
    const batch = writeBatch(db);

    querySnapshot.docs.forEach((docSnap) => {
      if (!snapshotMap.has(docSnap.id)) {
        batch.delete(docSnap.ref);
      }
    });

    snapshotMap.forEach((event, eventId) => {
      const basePayload = {
        title: event.title || '',
        date: event.date || '',
        backgroundColor: event.backgroundColor || '#a855f7',
      ...
      if (existingMap.has(eventId)) {
        batch.update(existingMap.get(eventId), basePayload);
      } else {
        const newRef = doc(collection(db, 'events'));
        batch.set(newRef, { ...basePayload, id: eventId });
      }
    });
  ...
  const undoLastChange = async () => {
    ...
    try {
      await restoreEventSnapshot(previous);
    } catch (error) {
      ...
      <div className="max-w-5xl mx-auto bg-white dark:bg-gray-800 dark:border dark:border-gray-700 shadow-xl rounded-2xl overflow-hidden relative transition-colors duration-200">
        ...
        <div className="p-6">
          ...
          <FullCalendar
            ...
            headerToolbar={{
              left: 'prev,next customUndo,today',
              center: 'title',
              right: 'dayGridMonth'
            }}
