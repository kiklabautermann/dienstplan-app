...
function App() {
  ...
  // Für mobile Touch-Geräte
  const [lastClick, setLastClick] = useState({ dateStr: '', time: 0 });
  ...
  // Undo stack: einfache Rückgängig-Implementierung für die letzte DB-Aktion
  // Eintrag: { op: 'delete'|'create'|'update', id: string, data?: object }
  const [undoStack, setUndoStack] = useState([]);
  const pushUndo = (entry) => setUndoStack(s => [...s, entry]);

  const findEventById = (id) => events.find(ev => ev.id === id || ev.originalId === id);
  ...
  const handleUndo = async () => {
    ...
    // Optimistisch Stack kürzen
    setUndoStack(s => s.slice(0, -1));

    try {
      if (entry.op === 'delete') {
        // undo für create: lösche das neu angelegte Dokument
        await deleteDoc(doc(db, 'events', entry.id));
      } else if (entry.op === 'create') {
        // undo für delete: lege das alte Dokument wieder an (mit selben ID)
        await setDoc(doc(db, 'events', entry.id), entry.data);
      } else if (entry.op === 'update') {
        // undo für update: setze die alten Felder zurück
        await updateDoc(doc(db, 'events', entry.id), entry.data);
      }

      await fetchEvents();
      alert('Rückgängig: Aktion wurde zurückgesetzt.');
    } catch (err) {
      console.error('Fehler beim Rückgängig machen:', err);
      alert('Fehler beim Rückgängig machen. Siehe Konsole.');
    }
  };
  ...
  if (!user) {
    return <Login />; // Bypassed for testing
  }
  ...
  const handleDateClick = async (arg) => {
    ...
    if (isTouch) {
      const now = Date.now();
      if (lastClick.dateStr === arg.dateStr && (now - lastClick.time) < 400) {
        // Double-tap detected
        setLastClick({ dateStr: '', time: 0 });
      } else {
        // First tap: save it and show user-friendly feedback
        setLastClick({ dateStr: arg.dateStr, time: now });
        showToast('Doppeltippen zum Eintragen');
        return;
      }
    }

    if (stampMode) {
      if (!window.confirm(`${stampMode.label} am ${arg.dateStr} eintragen?`)) return;
      try {
        ...
        // Für Undo: lösche dieses neu erzeugte Dokument
        pushUndo({ op: 'delete', id: docRef.id });
        await fetchEvents();
      } catch (error) {
      ...
      setPendingDate(arg.dateStr);
    }
  }

  const openNewEvent = (date) => {
    setPendingDate(null);
    setCurrentEvent({ id: null, title: '', date, backgroundColor: '#a855f7', borderColor: '#a855f7', comment: '', recurrence: 'none', emoji: '' });
    setModalOpen(true);
  };
  ...
  const handleEventDrop = async (info) => {
    ...
    try {
      // Für Undo: vorheriges Datum sichern
      const prev = findEventById(eventId);
      if (prev) {
        pushUndo({ op: 'update', id: eventId, data: { date: prev.date } });
      }
  ...
  const saveEvent = async (e) => {
    ...
    try {
      if (currentEvent.id) {
        ...
        // Für Undo: vorherige Werte sichern
        const prev = findEventById(currentEvent.id);
        if (prev) {
          pushUndo({ op: 'update', id: currentEvent.id, data: {
            title: prev.title,
  ...
  const deleteEvent = async () => {
    ...
    try {
      const prev = findEventById(currentEvent.id);
      if (prev) {
        // Für Undo: wieder anlegen mit gleicher ID
        pushUndo({ op: 'create', id: currentEvent.id, data: prev });
      }
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-4 md:p-8 transition-colors duration-200">
      <div className="max-w-5xl mx-auto bg-white dark:bg-gray-800 dark:border dark:border-gray-700 shadow-xl rounded-2xl overflow-hidden relative transition-colors duration-200">
              ...
              {eventCategories.map((cat, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 lg:px-4 py-2 lg:py-1.5 rounded-lg text-sm font-medium transition-all cursor-pointer border min-h-[2.75rem] lg:min-h-0 flex items-center justify-center ${isActive ? 'ring-2 ring-offset-1 dark:ring-offset-gray-800' : 'opacity-80 hover:opacity-100'}`}
                    style={{
        ...
        {/* Calendar */}
          <div className="p-4 md:p-6">
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4 italic">
              <span className="hidden lg:inline">Tipp: Tippe einen Termin zum Bearbeiten an. Tippe einen leeren Tag an und bestätige anschließend, um einen neuen Termin anzulegen.</span>
              <span className="lg:hidden">Tipp: Tippe einen Termin zum Bearbeiten an. Doppeltippe einen leeren Tag, um einen Termin anzulegen (Fehlklickschutz).</span>
            </p>
            {pendingDate && (
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900 dark:border-blue-800 dark:bg-blue-950/40 dark:text-blue-100">
                <span>Tag {pendingDate} ausgewählt. Termin wirklich anlegen?</span>
                <div className="flex gap-2">
                  <button type="button" onClick={() => openNewEvent(pendingDate)} className="min-h-11 rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white hover:bg-blue-700">Termin anlegen</button>
                  <button type="button" onClick={() => setPendingDate(null)} className="min-h-11 rounded-lg border border-blue-300 px-4 py-2 font-semibold text-blue-800 hover:bg-blue-100 dark:border-blue-700 dark:text-blue-200 dark:hover:bg-blue-900/50">Abbrechen</button>
                </div>
              </div>
            )}
          <FullCalendar
            ...
            customButtons={{
              undo: { text: '↩️ Rückgängig', click: handleUndo },
              addEvent: {
                ...
                click: () => {
                  ...
                  setCurrentEvent({ id: null, title: '', date: `${y}-${m}-${d}`, backgroundColor: '#a855f7', borderColor: '#a855f7', comment: '', recurrence: 'none', emoji: '' });
                  setModalOpen(true);
                }
              }
            }}
              ...
              return (
                <div className="flex justify-between items-start w-full px-1 pt-1 box-border">
                  <div className="flex-shrink-0">
                    {weather && (
                      <div 
                        className="flex items-center bg-transparent lg:bg-white lg:dark:bg-gray-800 lg:border border-gray-200 dark:border-gray-600 lg:shadow-sm rounded lg:px-1 py-0.5" 
                        title={`Max: ${weather.max}°C, Min: ${weather.min}°C`}
                      >
                        <span className="text-xs lg:text-sm leading-none">{weather.icon}</span>
                        <span className="hidden lg:flex flex-col text-[9px] font-bold leading-none ml-1">
            ...
            eventContent={(eventInfo) => {
              ...
              return (
                <div className={`group relative flex items-center gap-1 overflow-hidden w-full px-1.5 py-1 lg:py-0.5 min-h-[2.2rem] lg:min-h-0 ${darkMode ? 'text-gray-900' : 'text-white'}`}>
                  {hasComment && (
                    <span className="text-[10px] flex-shrink-0" title="Kommentar vorhanden">💬</span>
                  )}
                  <span className="truncate text-xs lg:text-sm font-semibold pr-1 lg:pr-4">
        ...
        {toast && (
          <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50 bg-gray-900/90 dark:bg-gray-800/95 text-white text-xs lg:text-sm px-4 py-2.5 rounded-xl shadow-lg border border-gray-700/50 flex items-center gap-2 animate-bounce">
            <span>💡</span>
            <span>{toast}</span>
          </div>
        )}
